...
if selected_kabupaten and selected_kecamatan and selected_vegetasi:
    ...
    if chart_type == "Pertumbuhan Vegetasi":
        ...
        if not selected_params_combined:
            ...
            # correlation_data = chart_data[['Pertumbuhan (%)'] + selected_params_combined].corr()
            ...
            #     text_auto=True,
            #     aspect="auto",
            #     title="Matriks Korelasi: Pertumbuhan Vegetasi vs Parameter Cuaca",
            ...
            # pertumbuhan_corr = correlation_data['Pertumbuhan (%)'].drop('Pertumbuhan (%)')
            
            # col1, col2 = st.columns(2)
            
            # with col1:
            #     st.write("**Korelasi Positif Terkuat:**")
            #     positive_corr = pertumbuhan_corr[pertumbuhan_corr > 0].sort_values(ascending=False)
            #     if not positive_corr.empty:
            #         top_positive = positive_corr.iloc[0]
            #         param_name = available_params.get(positive_corr.index[0], positive_corr.index[0])
            #         st.success(f"**{param_name}**: {top_positive:.3f}")
            #         st.write("Semakin tinggi parameter ini, semakin tinggi pertumbuhan vegetasi")
            #     else:
            #         st.info("Tidak ada korelasi positif yang signifikan")
            
            # with col2:
            #     st.write("**Korelasi Negatif Terkuat:**")
            #     negative_corr = pertumbuhan_corr[pertumbuhan_corr < 0].sort_values()
            #     if not negative_corr.empty:
            #         top_negative = negative_corr.iloc[0]
            #         param_name = available_params.get(negative_corr.index[0], negative_corr.index[0])
            #         st.error(f"**{param_name}**: {top_negative:.3f}")
            #         st.write("Semakin tinggi parameter ini, semakin rendah pertumbuhan vegetasi")
            #     else:
            #         st.info("Tidak ada korelasi negatif yang signifikan")
