...
KECAMATAN_LIST = [
    "Bandung Kota", "Cimahi", "Lembang", "Parongpong", 
    "Cisarua", "Sukasari", "Cicendo", "Coblong"
]
...
# Dropdown pemilihan kecamatan dan vegetasi
selected_kecamatan = st.sidebar.selectbox(
    "Pilih Kecamatan:",
    KECAMATAN_LIST,
    help="Pilih kecamatan yang ingin dianalisis"
)
...
if selected_kecamatan and selected_vegetasi:
    st.header(f"📊 Dashboard: {selected_vegetasi} di {selected_kecamatan}")
    ...
    if chart_type == "Pertumbuhan Vegetasi":
        fig = px.line(
            ...
            x='Tanggal', 
            y='Pertumbuhan (%)',
            title=f'Pertumbuhan {selected_vegetasi} di {selected_kecamatan}',
        ...
        fig.update_layout(
            title=f'Data Cuaca di {selected_kecamatan}',
            xaxis_title='Tanggal',
        ...
        fig.update_layout(height=600, title_text=f"Analisis Gabungan - {selected_kecamatan}")
        st.plotly_chart(fig, use_container_width=True)
...
if st.button("🔄 Siapkan Dataset untuk Diunduh", type="primary"):
    if download_start >= download_end:
        ...
        with st.spinner("Menyiapkan dataset..."):
            ...
            # Nama file
            filename = f"{dataset_type}_{download_kecamatan}_{download_start}_{download_end}.csv"
